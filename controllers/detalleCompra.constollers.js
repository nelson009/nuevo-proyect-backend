...
const detalleCompra = async (req, res) => {
    if(!req.user) {
        return res.status(200).json('sesion expirada del usuario')
    }
    ...
    // const totalPrecio = listarProductos.reduce((acc, ele) =>  acc + ele.precio,0)
    ...
    return res.status(200).json( await carrito.deleteCarrito(idCarrito.toString(),req));
};
